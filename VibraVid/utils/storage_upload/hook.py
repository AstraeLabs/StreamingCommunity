            ...
            bm.handle_progress_line(
                {
                    ...
                    "size": (
                        f"{internet_manager.format_file_size(done)}/{internet_manager.format_file_size(total) if total else '?'}"
                    ),
...
def try_fetch(output_path: str) -> bool:
    ...
    try:
        ...
        got = _run_with_bar(
            "[green]Cache[/green] [cyan]Downloading[/cyan]",
            lambda cb: upload_vault.download(hit["xh"], output_path, on_progress=cb),
        )
        if got:
...
def upload_after(output_path: str) -> None:
    ...
    try:
        ...
        _run_with_bar(
            "[green]Cache[/green] [cyan]Uploading[/cyan]",
            lambda cb: upload_vault.upload(
                output_path,
                title=title,
