...
class BinaryPaths:
    ...
    def _download_direct_release(self, tool: str, binary_name: str) -> str | None:
        """Handle direct-release downloads for tools not present in the manifest."""
        tool_key = str(tool).lower().replace("-", "_")
        binary_key = str(binary_name).lower()

        if tool_key in {"yt_dlp", "yt-dlp"} and binary_key in {"yt-dlp", "yt-dlp.exe"}:
            target_dir = self.get_binary_directory()
            ...
            try:
                ...
                logger.warning(f"Failed to download yt-dlp direct release: {exc}")
                return None
            if os.path.isfile(target_path):
                if self.system != "windows":
                    os.chmod(target_path, 0o755)
                self._resolved[binary_name] = target_path
                return target_path
            return None

        if tool_key in {"deno", "denojs", "deno_js"} and binary_key in {"deno", "deno.exe"}:
            target_dir = self.get_binary_directory()
            os.makedirs(target_dir, exist_ok=True)
            target_path = os.path.join(target_dir, binary_name)
            if self.system == "windows":
                ...
                arch_suffix = "x86_64" if self.arch == "x64" else "aarch64"
            ...
            zip_path = os.path.join(self.get_binary_directory(), "deno_temp.zip")
            extract_dir = os.path.join(self.get_binary_directory(), "temp_deno")
            try:
                ...
                for root, _, files in os.walk(extract_dir):
                    for name in files:
                        low = name.lower()
                        if low in {"deno", "deno.exe"}:
                            extracted_candidates.append(os.path.join(root, name))
                if not extracted_candidates:
                    raise FileNotFoundError("No deno binary found in archive")
                chosen = extracted_candidates[0]
                os.makedirs(os.path.dirname(target_path), exist_ok=True)
                shutil.move(chosen, target_path)
                if self.system != "windows":
                    os.chmod(target_path, 0o755)
                self._resolved[binary_name] = target_path
                return target_path
            except Exception as exc:  # pragma: no cover - network fallback path
                logger.warning(f"Failed to download Deno direct release: {exc}")
                return None
            finally:
                for stale_path in (zip_path, extract_dir):
                    try:
    ...
    def download_binary(self, tool: str, binary_name: str) -> str | None:
        ...
        with self._download_lock:
            ...
            direct_path = self._download_direct_release(tool, binary_name)
            if direct_path:
                return direct_path
