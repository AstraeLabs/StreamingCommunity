...
def _resolve_direct_download_root() -> Path:
    """Return the canonical app-local folder for yt-dlp direct downloads: OUTPUT_ROOT/MyDownloader."""
    try:
        root_value = config_manager.config.get("OUTPUT", "root_path", default="Video")
    except Exception:
        root_value = "Video"

    root_path = Path(str(root_value).strip() or "Video").expanduser()
    if not root_path.is_absolute():
        root_path = (Path.cwd() / root_path).resolve()

    if root_path.name.lower() in {"mydownloader", "mydownload"}:
        target_dir = root_path
    elif root_path.name.lower() == "video":
        target_dir = root_path / "MyDownloader"
    else:
        target_dir = root_path / "MyDownloader"
...
def try_download_with_ytdlp_direct(
    ...
    requested_out_dir = str(out_dir).strip() if out_dir is not None else ""
    ...
    if requested_out_dir:
        ...
        if requested_path.name.lower() in {"mydownloader", "mydownload"}:
            out_dir_path = requested_path
        elif requested_path.name.lower() == "video":
            out_dir_path = requested_path / "MyDownloader"
