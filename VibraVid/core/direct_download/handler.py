...
def handle_direct_download_isolated(args) -> tuple[bool, bool]:
    ...
    if out_root_path.name.lower() in {"mydownloader", "mydownload"}:
        out_dir = str(out_root_path)
    elif out_root_path.name.lower() == "video":
        out_dir = str(out_root_path / "MyDownloader")
    else:
        out_dir = str(out_root_path / "MyDownloader")
